...
/// @brief Check if an elf's duties are a subset of the other
/// @param elfASet The duties of elf A
/// @param elfBSet The duties of elf B
/// @return Whether the one elf's duties were a subset of the other
bool isSubset(elf_set_t elfASet, elf_set_t elfBSet) {
    if (elfASet.start <= elfBSet.start && elfASet.end >= elfBSet.end) {
        return true;
    }
    if (elfBSet.start <= elfASet.start && elfBSet.end >= elfASet.end) {
        return true;
    }
    return false;
}
...
int main(int argc, char *argv[]) {
    ...
    while (true) {
        ...
        if (isSubset(elfSetA, elfSetB)) { subsetCount += 1; }
