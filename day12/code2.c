...
unsigned int find_shortest_path_length(map_t map, nodepool_t starts, node_t end, nodepool_t *nodepool, unsigned int generation) {
    ...
    if (new_nodepool.num_nodes == 0) { return (map.height * map.width) + 1; }
    ...
    generation++;
    unsigned int generations = find_shortest_path_length(map, new_nodepool, end, nodepool, generation);
...
void fix_map_endpoints(map_t *map, node_t start, node_t end) {
    map->map[start.x][start.y] = 'a';
    map->map[end.x][end.y] = 'z' + 1;
}


/// @brief Get all the nodes with a certain symbol (character)
/// @param map The map to search
/// @param symbol The symbol (character) to look for
/// @return A nodepool of start points to check
nodepool_t get_all_nodes(map_t map, char symbol) {
    node_t *nodes = malloc(map.height * map.width * sizeof(node_t));
    nodepool_t nodepool = {0, nodes};
    for (unsigned int row_index = 0; row_index < map.height; row_index++) {
        for (unsigned int col_index = 0; col_index < map.width; col_index++) {
            if (map.map[row_index][col_index] == symbol) {
                node_t start_node = {row_index, col_index};
                nodepool.pool[nodepool.num_nodes] = start_node;
                nodepool.num_nodes++;
            }
        }
    }
...
int main(int argc, char **argv) {
    ...
    nodepool_t start_nodes = get_all_nodes(map, 'a');
    print_nodepool(start_nodes);
    unsigned int shortest_length = (map.height * map.width) + 1;

    for (unsigned int node_index = 0; node_index < start_nodes.num_nodes; node_index++) {

        start = start_nodes.pool[node_index];
        ...
        unsigned int current_length = find_shortest_path_length(map, nodepool, end, &nodepool, 1);
        shortest_length = current_length < shortest_length ? current_length : shortest_length;

        // Free the memory allocated for this start position
        free(checked_nodes);
