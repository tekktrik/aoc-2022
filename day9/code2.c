...
/// @param rope Pointer to the rope
/// @param position_list Pointer to the position list
/// @param instruction The current instruction of which this step is a part
void execute_step(rope_t *rope, rope_pos_list_t *position_list, move_inst_t instruction) {
    move_rope_head(rope->knots[0], instruction);
    bool knot_moved = true;
    for (int knot_index = 1; knot_index < rope->length; knot_index++) {
        knot_moved = move_knot(rope->knots[knot_index], rope->knots[knot_index - 1]);
        if (!knot_moved) { break; }
    }
    
    // Record if tail moved
    record_tail_position(rope->knots[rope->length - 1], position_list, knot_moved);
}
...
/// @param rope Pointer to the rope
/// @param position_list Pointer to the position list
void execute_instruction(move_inst_t instruction, rope_t *rope, rope_pos_list_t *position_list) {
    for (long move_index = 0; move_index < instruction.num_steps; move_index++) {
        execute_step(rope, position_list, instruction);
    }
}
...
/// @param argv The arguments provided ([Program name, Input filepath, Rope length])
...
int main(int argc, char **argv) {
    ...
    // Create the rope
    char *end_ptr;
    unsigned int rope_length = strtol(argv[2], &end_ptr, 10);
    knot_t **knots = malloc(rope_length * sizeof(knot_t *));
    rope_t rope = {rope_length, knots};
    ...
    while (true) {
        ...
        execute_instruction(next_instruction, &rope, &position_list);
