...
/// @param lcm The least common multiple of the monkeys test divisors
void simulate_round(monkey_t ***monkeys, unsigned int num_monkeys, unsigned long long lcm) {
    for (unsigned int monkey_index = 0; monkey_index < num_monkeys; monkey_index++) {
        ...
        for (unsigned int item_index = 0; item_index < current_monkey->num_items; item_index++) {
            unsigned long long new_item_level = current_monkey->items[item_index] % lcm;
            new_item_level = update_worry_level(*current_monkey, new_item_level);
...
int main(int argc, char **argv) {
    ...
    // Get the LCM of the monkey test divisors
    unsigned long long test_lcm = 1;
    for (int monkey_index = 0; monkey_index < num_monkeys; monkey_index++) {
        test_lcm *= monkeys[monkey_index]->test_divisor;
    }
    ...
    for (unsigned int round_index = 0; round_index < num_rounds; round_index++) {
        simulate_round(&monkeys, num_monkeys, test_lcm);
    }
