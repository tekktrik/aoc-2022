...
wall_map_t * create_wall_map(element_node_t **segments, unsigned int num_segments) {
    ...
    unsigned int height = (max_y - min_y) + 2;
...
bool simulate_sand(wall_map_t *wall_map, element_node_t start_node) {
    ...
    while (true) {
        ...
        // Check if at abyss level
        if (y_index == wall_map->height - 1) { return false; }
        ...
        wall_map->map[x_index][y_index] = (x_index + wall_map->min_x == start_node.x) && (y_index + wall_map->min_y == start_node.y) ? ENTRY : SPACE;
...
int main(int argc, char **argv) {
    ...
    unsigned long units_sand = 0;
    while (simulate_sand(wall_map, sand_node)) { units_sand++; } 

    // Print the stable map and amount of sand dropped
    print_map(*wall_map);
